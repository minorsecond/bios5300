...
results$chsq.by.sex <- svychisq(~diag.stroke+sex, 
                                design = brfss.survey.design)
...
results$income.prob.graph <- ggplot(ggpredict(results$stroke.qbinom, 
                                              terms=c("income", "diag.depression")), 
                                    aes(factor(x, 
                                               labels = c("Income < $10,000",
                                                          "Income < $15,000",
                                                          "Income < $20,000",
                                                          "Income < $25,000",
                                                          "Income < $35,000",
...
results$days.anxious.scatter <- ggplot(results$days.anxious.pred, 
  ...
  labs(x="Days Anxious in Past Month", 
       y = "Stroke Probability", 
       caption = "") +
  theme_Publication() +
  scale_colour_Publication()
...
results$total.income.groups <- svytotal(~income, 
                                        design = brfss.survey.design, 
                                        na.rm=T)
results$total.income.groups <- melt(results$total.income.groups)
colnames(results$total.income.groups) <- c("Total", 
                                           "SE")
...
# Age group bargraph
results$total.age.groups <- svytotal(~age.grp, 
                                     design = brfss.survey.design, 
                                     na.rm=T)

results$total.age.groups <- melt(results$total.age.groups)
colnames(results$total.age.groups) <- c("Total", 
                                        "SE")
results$total.age.groups <- setDT(results$total.age.groups, 
                                  keep.rownames = TRUE)[]

results$bargraph_age.grp <- ggplot(results$total.age.groups, aes(rn, 
                                                                 Total)) +
  geom_bar(stat = "identity") +
  theme_Publication() +
  scale_colour_Publication() +
...
us_states@data <- join(as.data.frame(us_states), 
                       census, "STATE")
us_states@data <- join(as.data.frame(us_states), 
                       results$total.strokes.by.state, "STATE")
us_states$id <- rownames(us_states@data)
us_states$stroke.prevalence <- (us_states$diag.strokeStroke / us_states$Pop.2016) * 100000
stroke.prevalence <- na.omit(us_states[,c("STATE", 
                                          "stroke.prevalence")]@data)
...
us_states.gg <- join(us_states.gg, 
                     us_states@data, 
                     by="id")

results$colorPalette <- brewer.pal(5, 
                                   name = "YlGn")
results$map.stroke.prevalence <- ggplot(stroke.prevalence, 
                                        aes(map_id = STATE)) + 
  geom_map(aes(fill = quintile), 
           map = fifty_states, 
           show.legend = T, 
           colour="grey25", 
           size = .25) + 
